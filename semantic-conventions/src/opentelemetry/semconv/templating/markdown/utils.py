...
class VisualDiffer:
    ...
    def __init__(self, logger):
        self._log_func = logger or (lambda text: print(text))
    ...
    def visual_diff(cls, a: str, b: str):
        ...
        """
        if not os.environ.get("COLORED_DIFF"):
            # return "\n".join(difflib.context_diff(a, b))
            added = lambda text: f"{{added:'{text}'}}"
            removed = lambda text: f"{{removed:'{text}'}}"
        else:
            added = lambda text: cls.colorize_text(0, 255, 0, text)
            removed = lambda text: cls.colorize_text(255, 0, 0, text)
        ...
        for operation, a_start, a_end, b_start, b_end in diff_partitions.get_opcodes():
            if operation == "equal":
                ...
                colored_diff.append(added(b[b_start:b_end]))
            elif operation == "delete":
                colored_diff.append(removed(a[a_start:a_end]))
        ...
        colored_diff = "".join(colored_diff)
        return colored_diff
